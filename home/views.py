...
def shorten_url_form(request):
    ...
    if request.method == "POST":
        ...
        # check back half already present
        sobj = url_shortener.objects.all()
        back_half_list = []
        for x in sobj:
...
def redirect_to_url(request, key):
    ...
    try:
        sobj = url_shortener.objects.get(
            back_half=key)  # select url from back-half
        url = sobj.url
        return redirect(url)
    except:
...
def shorten_code_form(request):
    ...
    if request.method == "POST":
        ...
        description = request.POST.get("description")
        # check back half already present
        sobj = code_shortener.objects.all()
        back_half_list = []
        for x in sobj:
            back_half_list.append(x.back_half)
        if back_half in back_half_list:
